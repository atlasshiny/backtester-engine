...
class BacktestEngine():
    ...
    def results(self, plot: bool = True, save: bool = False, risk_free_rate: float = 0.0):
            ...
            - Trade statistics
            - Equity curve and price plots
            """

            # convert value history to numpy array for calculations 
            equity = np.array(self.portfolio.value_history)
            returns = np.diff(equity) / equity[:-1]

            # basic metrics 
            final_value = equity[-1]
            pnl = final_value - self.portfolio.initial_cash
            ...
            if plot:
                ...
                ax2 = ax1.twinx()
                ax2.plot(self.data_set['Close'].values, color='tab:orange', label='Price (Close)')
