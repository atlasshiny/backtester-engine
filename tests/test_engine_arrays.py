...
def make_single_asset_arrays(drop_date=False):
    df = pd.DataFrame({
...
class TestEngineArrays(unittest.TestCase):
    def test_multi_asset_row_by_row_arrays(self):
        arrays = make_multi_asset_arrays()
        strat = DummyStrategy()
        port = Portfolio(200)
        broker = Broker(port)
        engine = BacktestEngine(strat, port, broker, arrays, warm_up=0, group_by_date=False)
        engine.run()
    ...
    def test_multi_asset_group_by_date_arrays(self):
        arrays = make_multi_asset_arrays()
        strat = DummyStrategy()
        port = Portfolio(200)
        broker = Broker(port)
        engine = BacktestEngine(strat, port, broker, arrays, warm_up=0, group_by_date=True)
        engine.run()
        self.assertEqual(set(p.qty for p in port.positions.values()), {1} if port.positions else set())
        self.assertEqual(len(broker.trade_log), 6)

    def test_symbol_injection_for_single_asset_arrays(self):
        arrays = make_single_asset_arrays(drop_date=True)
        strat = DummyStrategy()
        port = Portfolio(100)
        broker = Broker(port)
        engine = BacktestEngine(strat, port, broker, arrays, warm_up=0, group_by_date=False)
        engine.run()
        # all trades with qty>0 should have symbol 'SINGLE'
